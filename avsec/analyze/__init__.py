...
def run_frustum_spoof_analysis(DM, result_path, data_dict_in, baseline_data=None):
    """
    ...
    # Analyze data
    results, conf_mat = evaluation.get_detection_results_from_folder(
        DM, result_path, metric="3D_IoU", whitelist_types=["Car"]
    )
    idx_result_start = 0
    ...
    for i_run in range(len(data_dict["frame"])):
        ...
        try:
            ...
            for j_instance in range(n_this_run):
                attack_xyz = data_dict["attack_xyz"][i_run][
                    np.unravel_index(j_instance, mesh_shape)
                ]
                ...
                (
                    ...
                    fp_near_inject_v,
                    fn_of_target_v,
                ) = _single_frustum_attack_analysis(result_this, attack_xyz, target_xyz)
            ...
            nassign, nfp, nfn, fp_near_inject, fn_of_target = [
                np.reshape(l, mesh_shape)
                for l in [nassign, nfn, nfn, fp_near_inject, fn_of_target]
            ]
...
def _single_frustum_attack_analysis(
    result, attack_xyz, target_xyz, thresh_for_fp=5, thresh_for_fn=0.1
):
    ...
    thresh_for_fp = 5
    fp_near_inject = np.any(
        [np.linalg.norm(fp.box3d.t - attack_xyz) < thresh_for_fp for fp in fp_this]
    )
    # Get boolean if FN exists on target vehicle
    thresh_for_fn = 0.1
    fn_of_target = np.any(
        [np.linalg.norm(fn.box3d.t - target_xyz) < thresh_for_fn for fn in fn_this]
    )
...
def run_naive_spoof_analysis(DM, result_path, data_dict_in, baseline_data=None):
    ...
    for i_run in range(len(data_dict["frame"])):
        ...
        if baseline_data is not None:
            # Get values
            nFP_baseline = baseline_data[data_dict["frame"][i_run]].get_number_of(
                "false_positives"
            )
            nFN_baseline = baseline_data[data_dict["frame"][i_run]].get_number_of(
                "false_negatives"
            )
            # Lowest is 0
...
def heat_wrapper(ax, xm, ym, data_dict, idx, index_string, vmin, vmax):
    ...
    d = {
        xstr: xm.flatten(),
        ystr: ym.flatten(),
        index_string: data_dict[index_string][idx].flatten(),
    }
    df = pd.DataFrame(data=d)
    table = df.pivot(ystr, xstr, index_string)
    heatmap_data(xm, ym, table, title=index_string, vmin=vmin, vmax=vmax, ax=ax)


def heatmap_data(
    xdata, ydata, table, title, vmin=0, vmax=1, nxlab=10, nylab=10, ax=None
):
    if vmin < 0:
        cmap = "seismic"
    else:
