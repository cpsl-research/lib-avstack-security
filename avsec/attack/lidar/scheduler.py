...
class ReplayScheduler(Scheduler):
    ...
    def __init__(
        self, dt_burnin=0, dt_stable=3, dt_attack=3, framerate=5, dt_repeat=0.5
    ):
...
class _NaiveMoveObjectScheduler(Scheduler):
    ...
    def _schedule_stable(self, info):
        ...
        pts_ground = tforms.spherical_to_cartesian(
            pts_razel, geometry.StandardCoordinates
        )  # always in standard coordinates
    ...
    def _schedule_attack(self, info):
        ...
        pts_ground_final = tforms.spherical_to_cartesian(
            self.final_razel, geometry.StandardCoordinates
        )  # always in standard coordinates
        n_frames_to_go = self.n_frames_attack - (
            self.n_frames_ready - self.n_frames_burnin - self.n_frames_stable
        )
        if n_frames_to_go <= 0:
            ...
            if self.attack_profile == "linear":
                ...
                elif (n_frames_to_go <= self._switch_at_n_frames) and (
                    self._mode == "init"
                ):
                    self._mode = "final"
                ...
                pts_ground = pts_ground_last + vector * (
                    v * self.dt + 1 / 2 * self._a * self.dt**2
                )
...
class _FrustumMoveObjectScheduler(Scheduler):
    ...
    def __str__(self):
        return (
            f"Frustum object move scheduler: initial range:"
    ...
    def _get_track_box(self, tracks, track_scores):
        ...
        if self.trk_ID is None:
            if (
                (len(tracks) == 0)
                or np.all(np.isinf(track_scores))
                or np.isnan(track_scores).all()
            ):
                return None, None
            else:
        ...
        for trk in tracks:
            if (trk.ID == self.trk_ID) or (
                (self.last_trk_target is not None)
                and (np.linalg.norm(trk.position - self.last_trk_target.position) < 1)
            ):
                trk_box = trk.box3d
    ...
    def _schedule_stable(self, info):
        ...
        if trk_box is None:
            ...
            pts_ground = np.array(
                [s * trk_position[0], s * trk_position[1], 0]
            )  # HACK for now
            self.last_pts_ground = pts_ground
            info["pts_ground"] = [pts_ground]
            info["attack_target"] = [pts_ground]
    ...
    def _schedule_attack(self, info):
        ...
        if self.delta_attack is None:
            ...
            self.delta_attack = (
                self.pts_ground_final - pts_ground_last
            ) / self.n_frames_attack
        ...
        if n_frames_to_go <= 0:
            ...
            if self.maintain_frustum:
                trk_box, xyz_size = self._get_track_box(
                    info["tracks"], info["track_scores"]
                )
                info["track_box"] = trk_box
                trk_position = trk_box.t
                if self.attack_profile == "linear":
                    s = np.linalg.norm(
                        pts_ground_last + self.delta_attack
                    ) / np.linalg.norm([trk_position[:2]])
                    pts_ground = np.array(
                        [s * trk_position[0], s * trk_position[1], 0]
                    )  # HACK for now
                    ...
                    # -- position
                    vector = np.array(
                        [trk_position[0], trk_position[1], 0]
                    ) / np.linalg.norm(
                        trk_position[:2]
                    )  # HACK for now
                    delta = (
