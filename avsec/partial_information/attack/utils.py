...
def sample_pc(pc, keep, strategy="random", upsample=False):
    ...
    if (pts_choose > pc2.shape[0]) and not upsample:
        warnings.warn(
            "Number of desired points (%i) is more than the points that exist (%i)...going with max points"
...
def range_out_points(
    lidar, label, calib, r_new, gp_consistent=False, rescale=False, scale_range=False
):
    """
    ...
    box_center_velo = calib.project_rect_to_velo(np.expand_dims(box_center_rect, 0))[
        0, :
    ]
    bcv = box_center_velo
    ...
    box_center_rect = calib.project_velo_to_rect(np.expand_dims(box_center_velo, 0))[
        0, :
    ]
    ...
    if rescale:
        ...
        new_lidar[:, 0:3] = (new_lidar[:, 0:3] - box_center_velo) @ np.transpose(
            M
        ) + box_center_velo
        ...
        corners_scaled_velo = (box_corners_velo - box_center_velo) @ np.transpose(
            M
        ) + box_center_velo
...
class NaiveSpoofing(LidarAttack):
    ...
    def get_spoof_points(self, dist, npts, itrace):
        ...
        obj_lidar, obj_bbox = move_to_front(
            occ_obj_data["lidar"], occ_obj_data["bbox"], dist
        )
...
def run_full_spoofing_attack(
    ...
    if (alg_file_names_map is None) or (len(alg_file_names_map) < len(alg_names)):
        alg_file_names_map = make_default_file_names(
            alg_names, attack_type, inject_type, inject_shape, defense
        )

    # Preliminaries
    attack_params = get_attack_params(DM, attack_type)
    DEF_POSTPROC, svf, svf_method = get_defenses(defense)
    istarts, DCs_master = handle_checkpoint(
        out_folder, alg_file_names_map, attack_type, attack_params, use_checkpoint
    )
    ...
    for i_iter in range(istart_min, attack_params["params_loop"].shape[1], 1):
        ...
        if (attack_type == "frustum") and (
            len(
                DM.get_labels(
                    attack_params["params_loop"][:, i_iter][0], whitelist_types=["Car"]
                )
            )
            == 0
        ):
        ...
        # Make data
        DC_ADD_blank = make_data(
            DM, exp_split, i_iter, attack_params, attack_type, inject_type, inject_shape
        )
        ...
        for alg in algs_to_call:
            ...
            # --- call algorithm
            result_dir = call_percep_on_data(
                DM, exp_folder, exp_split, [alg], svf=svf, svf_method=svf_method
            )
            ...
            # --- run analysis
            DC_ADD = analysis_on_percep_results(
                DM,
                attack_type,
                exp_split,
                result_dir,
...
def get_attack_params(DM, spoof_type, **kwargs):
    if spoof_type == "naive":
        ...
        npts = np.concatenate(
            ([2], np.arange(5, 100, 5), np.arange(100, 210, 10)), axis=0
        )
...
def analysis_on_percep_results(
    DM, attack_type, exp_split, result_dirs, DC_ADD, results_baseline, algs
):
...
def call_postproc_defenses(DM, DEF_POSTPROC, exp_split, result_dirs):
    ...
    if DEF_POSTPROC is not None:
        ...
        for i, res_dir in enumerate(result_dirs):
            ...
            _, _, result_dirs[i] = DEF_POSTPROC.evaluate_from_folder(
                res_dir, write=True
            )
    return result_dirs
        ...
        for alg, fname in alg_file_names_map.items():
            ...
            if os.path.exists(fpath):
                ...
                if attack_type == "naive":
                    ...
                    aeq = np.all(
                        attack_params["params_loop"].T == frame_last_done, axis=1
                    )
                else:
                    raise NotImplementedError
...
def _handle_frustum_attack_inputs(
    DM,
    file_name,
    exp_name,
    algorithm,
    ...
    if use_checkpoint and (os.path.exists(fname_res)):
        ...
        if "defense" in fname_res:
            ...
            last_dist_finished = (
                data_collect_master["attack_dist"][-1]
                - data_collect_master["target_dist"][-1]
            )
...
def make_naive_spoofing_data(
    ...
    part_func = partial(
        _make_single_naive_data, DM, dataset_path, occ_obj_data, itrace, npts, distance
    )
...
def _make_single_naive_data(
    DM, dataset_path, occ_obj_data, itrace, npts, distance, count_frame_tup
):
    icount, frame = count_frame_tup
    calib = DM.get_calibration(frame)
    lidar_altered, attack_xyz_velo = create_inject_frame(
        DM, occ_obj_data, frame, npts, distance
    )
...
def create_inject_frame(DM, occ_obj_data, ifr, npts, distance):
    ...
    # Move object
    obj_lidar, obj_bbox = move_to_front(
        occ_obj_data["lidar"], occ_obj_data["bbox"], distance
    )
...
def make_frustum_sample_spoofing_data(
    DM,
    exp_split,
    npts,
    dist_rel,
    ...
    """
    dataset_path, data_collect = _validate_frustum_inputs(
        DM, exp_split, inject_type, seed
    )
    ...
    if os.path.exists(os.path.join(DM.split_path, "timestamps.txt")):
        shutil.copyfile(
            os.path.join(DM.split_path, "timestamps.txt"),
...
def _make_sample_frustum_data(
    DM, idxs_data, dataset_path, inject_type, npts, dist_rel, random_frames, seed, irun
):
    ...
    if random_frames:
        ...
        while True:
            ...
            if len(labels) > 0:
                idx_object = np.random.RandomState().choice(range(len(labels)), size=1)[
                    0
                ]
                # Check number of points
                lidar_pc = DM.get_lidar(idx_frame)
                calib = DM.get_calibration(idx_frame)
                mask_bbox = maskfilters.filter_points_in_object_bbox(
                    lidar_pc, labels[idx_object].box3d, calib
                )
                if sum(mask_bbox) > 0:
                    break
    else:
    ...
    if (not random_frames) and (len(labels) < (idx_object + 1)):
        ...
        (
            ...
            mask_bbox,
            lidar_orig_ex,
        ) = _get_single_data_sample(DM, idx_frame, idx_object, inject_type)
        lidar_attack_base = _make_frustum_attack_base(
            lidar_orig_ex, label_orig_ex, calib, inject_type, npts
        )

        # Make spoofing data (includes saving)
        params = (-1, irun, npts, dist_rel)
        _, attack_xyz = _make_single_frustum_data(
            DM,
            dataset_path,
            ground,
            labels_all,
...
def make_frustum_mesh_spoofing_data(
    DM,
    exp_split,
    idx_attack_indicators,
    npts,
    ...
    for i_comb, (idx_frame, idx_object) in enumerate(idx_attack_indicators):
        ...
        params = [
            (i_idx, i_idx + icount, npts, dist)
            for i_idx, (npts, dist) in enumerate(zip(xm.ravel(), ym.ravel()))
        ]
        icount += n_create
        part_func = partial(
            _make_single_frustum_data,
            DM,
            dataset_path,
            ground,
            labels_all,
...
def _get_single_data_sample(DM, idx_frame, idx_object, inject_type):
    ...
    region = (
        inject_type.replace("orig-", "")
        if inject_type in ["orig-bbox", "orig-frustum"]
        else "bbox"
    )
    try:
    ...
    mask_frustum = maskfilters.filter_points_in_frustum(
        lidar_pc_in, label_orig_ex.box2d, calib
    )
    mask_bbox = maskfilters.filter_points_in_object_bbox(
        lidar_pc_in, label_orig_ex.box3d, calib
    )
    lidar_orig_ex = (
        lidar_pc_in[mask_frustum, :]
        if region == "frustum"
        else lidar_pc_in[mask_bbox, :]
    )
    return (
        image,
        calib,
...
def _make_frustum_attack_base(lidar_orig_ex, label_orig_ex, calib, inject_type, npts):
    if inject_type in ["random-box"]:
        ...
        t_target_velo = calib.project_rect_to_velo(
            deepcopy(label_orig_ex.box3d.t)[:, None].T
        )[0, :]
        noise = (R_chol @ np.random.RandomState().randn(3, max_pts)).T
        lidar_pre_range = mean_loc + noise
        if len(lidar_orig_ex[:, 3]) > 0:
            lidar_intensity = np.random.RandomState().choice(
                lidar_orig_ex[:, 3], max_pts, replace=True
            )[:, None]
        else:
...
def _make_single_frustum_data(
    ...
    lidar_pc_ex = percep_attacks.lidar.sample_pc(
        lidar_pc_ex, keep=npts, upsample=allow_upsample
    )
    # --- move points
    ...
    # --- merge data
    attack_xyz = calib.project_velo_to_rect(
        np.mean(lidar_pc_ex[:, 0:3], axis=0)[:, None].T
    )
    lidar_pc_merged = np.concatenate((lidar_pc_in, lidar_pc_ex), axis=0)
    save_data(
        DM, calib, lidar_pc_merged, labels_all, image, ground, dataset_path, icount
    )
...
def save_data(DM, calib, lidar, labels, image, ground, dataset_path, icount):
    ...
    if ground is not None:
        DM.save_ground(
            [-ground[1], -ground[2], ground[0], ground[3] - 0.08],
            os.path.join(dataset_path, "planes"),
            icount,
        )
