...
def find_common_combos(dcm_list):
    ...
    for idcm, dcm in enumerate(dcm_list):
        ...
        frame_obj_double_dict.append(
            {
                frame: {
                    obj: [] for obj in obj_arr[np.argwhere(fr_arr == frame)].flatten()
                }
    ...
    for idcm, dcm in enumerate(dcm_list):
        ...
        for frame in frame_inter:
            ...
            if frame not in running_frame_obj_set_dict:
                ...
                running_frame_obj_set_dict[frame] = running_frame_obj_set_dict[
                    frame
                ].intersection(set(obj_for_this_frame))

    return running_frame_obj_set_dict, frame_obj_double_dict


def get_number_of_succeed_each_case(
    dcm_list, iframe, iobj, running_frame_obj_set_dict=None, frame_obj_double_dict=None
):
