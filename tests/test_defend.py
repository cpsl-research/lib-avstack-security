...
import os
import numpy as np
from avstack import datasets
from seca import defend
...
def test_LPD_class_folder_mode():
    ...
    new_results, passed, _ = LPDDEF.evaluate_from_folder(path_to_results, idxs=None, write=False)
    assert(len(passed) == 2)
    ...
    #----write all
    path_to_results = make_results(screw=True)
    new_results, passed, write_path = LPDDEF.evaluate_from_folder(path_to_results, idxs=None, write=True)
    assert(len(passed) == 2)
    assert(not np.any(passed[1]))
    assert(not np.any(passed[100]))
    ...
    #----only use index
    path_to_results = make_results()
    new_results, passed, _ = LPDDEF.evaluate_from_folder(path_to_results, idxs=1, write=False)
    assert(len(passed) == 1)
    assert(np.all(passed[1]))

    #----only use index and write
    path_to_results = make_results(screw=True)
    new_results, passed, write_path = LPDDEF.evaluate_from_folder(path_to_results, idxs=1, write=True)
    assert(len(passed) == 1)
    res = LPDDEF.get_detection_results_from_folder(write_path)
    assert(len(res) == 1)
    assert(len(res[1]) == 0)
...
def test_comapare_2d_3d_image_bbox():
    ...
    for lab in labels:
        inter, inter_ratio, union, union_ratio, iou, area_ratio = \
                                      defend.lidar.compare_2d_3d_image_bbox(lab.box3d,
                                                                        lab.box2d,
                                                                        calib,
                                                                        only_in_image=True)
        # Tests
        ...
        assert(iou > 0.7)
        assert((area_ratio > .7) and (area_ratio < 1.3))
